...
public class QLearningController extends Controller {
	...
	void performAction(int action) {
	    resetRockets(); // Always reset engines first!
	    ...
	    switch (action) {
	        ...
	        case 4: leftEngine.setBursting(true); middleEngine.setBursting(true); break;
	        case 5: rightEngine.setBursting(true); middleEngine.setBursting(true); break;
	        case 7: leftEngine.setBursting(true); rightEngine.setBursting(true); middleEngine.setBursting(true); break; // All engines
	    }
	}
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				//String prev_stateaction = previous_state + previous_action;  // Key for Q-table
				double oldQValue = Qtable.get(prev_stateaction); // current Q value

				double currentReward = StateAndReward.getRewardAngle(previous_angle, previous_vx, previous_vy);

				double maxQvalNextState = getMaxActionQValue(new_state); //max Q-value in next state over all possible actions

				// Using Bellman to update the Q-value of the new state-action pair
				double newQValue = oldQValue + alpha(Ntable.get(prev_stateaction)) * (currentReward + GAMMA_DISCOUNT_FACTOR*maxQvalNextState - oldQValue);
				Qtable.put(prev_stateaction, newQValue);
